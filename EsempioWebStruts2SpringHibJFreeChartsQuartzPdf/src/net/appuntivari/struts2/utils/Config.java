package net.appuntivari.struts2.utils;
...
public class Config {
	...
	private static synchronized void configInit() {
		try {
			...
			Config.load(new FileInputStream(ConstantsUtil.PROPERTIES_SERVER_FILE_PATH));
		} catch (Exception ex) {
			String msg = "Errore di caricamento dei file di properties [" + ConstantsUtil.PROPERTIES_SERVER_FILE_PATH + "]";
			//log.info(msg, ex);
		}
	}
	...
	private static String expand(String in){
		...
		try{
			...
			log.info("Reload FALLITO - Properties NON ALLINEATE");
		}
	...
	public static String getProp(String propName) {
		...
		if ((propValue = m_cfg.getProperty(propName)) == null) {
			//log.info("Configuration item [" + propName + "] missing!");
		}
		...
		//log.info("returning propName [" + propName + "] propValue [" + propValue + "]");
		return propValue;
	}
	...
	public static Properties getPropertiesObj() {
		if (m_cfg == null) {
			log.info("Properties object null!");
		}
	...
	public static Map<String, String> getAllProperties() throws FileNotFoundException, IOException {
	    ...
	    p.load(new FileInputStream(ConstantsUtil.PROPERTIES_SERVER_FILE_PATH));
	...
	public static void updateOldPropWithNewProp(String oldProp,String newProp) throws IOException {
	    ...
	    BufferedReader in = new BufferedReader(new FileReader(ConstantsUtil.PROPERTIES_SERVER_FILE_PATH));
	    ...
	    PrintWriter out = new PrintWriter(ConstantsUtil.PROPERTIES_SERVER_FILE_PATH);
	    for (String l : lines)
	        out.println(l);
	    out.close();
